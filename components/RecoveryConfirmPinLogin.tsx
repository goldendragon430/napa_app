...
const RecoveryConfirmPinLogin = ({route}: any) => {
  const {goBack, navigate} = useNavigation<any>();
  const toast = useToast();
  const [confirmPinCodeValue, setConfirmPinCodeValue] = useState<any>('');
  const dispatch = useDispatch();
  const {codeValue, emailAddress} = route?.params;
  ...
  const checkPin = async () => {
    if (confirmPinCodeValue.length == 6) {
      if (codeValue == confirmPinCodeValue) {
        console.log(confirmPinCodeValue);
        const token = await AsyncStorage.getItem('fcmToken');
        dispatch(setpinCode(codeValue));
  ...
  return (
    <ImageBackground
      ...
      <View
        ...
        <View>
          <Text style={styles.pinText}>Enter Confirm Pin Code</Text>
        </View>
...
export default RecoveryConfirmPinLogin;
