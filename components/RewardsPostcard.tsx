...
type RewardsPostcardProps = {
  item: any;
  index: number;
};
const RewardsPostcard: React.FC<RewardsPostcardProps> = ({item, index}) => {
  ...
  return (
    <TouchableOpacity onPress={()=>navigate(SCREENS.POSTDETAILS, {post: item?.postId})} style={styles.container}>
      ...
      <Text style={styles.likes}>{item?.awards}</Text>
    </TouchableOpacity>
  );
};

export default RewardsPostcard;
