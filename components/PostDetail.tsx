...
import {BackIcon, CrossIcon, DoubleDotIcon} from '../assets/svg';
...
import {getAllComments, getSingleSocialArtPost} from '../services/PostApi';
import {
  createNewComment,
  deleteComment,
  likeComment,
...
const PostDetail = ({route}: any) => {
  ...
  const [postDetails, setPostDetail] = useState<SinglePost>(post);
  const postId: any = postDetails?.postId;
  ...
  useEffect(() => {
    if (typeof post !== 'object') {
      const postData = async () => {
        const {data} = await getSingleSocialArtPost(post);
        setPostDetail(data);
      };
      postData();
    }
  ...
  const getComments = async () => {
    ...
    const {data, error} = await getAllComments(postDetails?.postId as string);
  ...
  const handleCreateComment = async () => {
    if (commentText.length >= 500) {
      setLoading(false);
      toast.show(
        <WarningToast message="Comment text can be no more than 500 characters" />,
        {
          placement: 'top',
        },
      );
    ...
    setLoading(true);
    const {error, message} = await createNewComment(comment as any);
    if (error) {
      setLoading(false);
        ...
        <View
          ...
          <ScrollView>
            <SocialArtPosts
              ...
              videoTitle={postDetails?.videoTitle}
              videoCaption={postDetails?.videoCaption}
              commentByUser={postDetails?.commentByUser}
              likedByUsers={postDetails?.likedByUsers}
              videoURL={
                Platform.OS === 'ios'
                  ? postDetails?.videoURL
                  : postDetails?.mobileVideoURL
              }
              views={postDetails?.views}
              setPostDetail={setPostDetail}
              postId={postDetails?.postId}
              minted={postDetails?.minted}
              mintedTimeStamp={postDetails?.mintedTimeStamp}
              createdAt={postDetails?.createdAt}
              accountId={postDetails?.accountId}
              isExpired={postDetails?.isExpired}
              profileId={postDetails?.profileId}
              videoThumbnail={postDetails?.videoThumbnail}
                              ...
                              <View style={styles.userProfile}>
                                ...
                                <Pressable
                                  onPress={() => {
                                    ...
                                    if (
                                      postComment?.item?.profileId == profileId
                                    ) {
                                      setIsVisible(true);
