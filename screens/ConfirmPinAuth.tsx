...
import {setIsLoggedIn, setpinCode} from '../store/slices/ProfileDetail';
import WarningToast from '../common/toasters/WarningToast';
import {setRegistrationType} from '../store/slices/CreateUserProfile';

const ConfirmPinAuth = ({route}: any) => {
  const {goBack, navigate} = useNavigation<any>();
  const toast = useToast();
  const [confirmPinCodeValue, setConfirmPinCodeValue] = useState<any>('');
  const {pinCodeValue} = route.params;
  ...
  const checkPin = async () => {
    if (confirmPinCodeValue.length == 6) {
      if (pinCodeValue === confirmPinCodeValue) {
        dispatch(setpinCode(pinCodeValue));
        // await AsyncStorage.setItem('registrationType', 'Pin');
        dispatch(setRegistrationType('Pin'));
        dispatch(setIsLoggedIn(true));
        navigate(SCREENS.CREATEPROFILE);
      } else {
        toast.show(<WarningToast message="Pin number did not match" />, {
          placement: 'top',
        });
  ...
  return (
    <ImageBackground
      ...
      <View
        ...
        <View>
          <Text style={styles.pinText}>Confirm 6-digit Pin Number</Text>
